...
class graphicl():
    ...
    def training(self):
        ...
        if self.args.setting == "natural":  
            os.makedirs(f'./results/our/{self.args.dataset}', exist_ok=True)
            text = open(f'./results/our/{self.args.dataset}/{current_time}.txt', 'w')
        else:  
            os.makedirs(f'./results/our/{self.args.dataset}/{self.args.imbalance_ratio}',
                        exist_ok=True)
            text = open(
                f'./results/our/{self.args.dataset}/{self.args.imbalance_ratio}/{current_time}.txt', 'w')
        ...
        for seed in range(5, 5 + self.args.num_seed):
            ...
            optimizer_fe = optim.Adam(model.encoder.parameters(), lr=self.args.lr, weight_decay=self.args.wd)  
            optimizer_cls = optim.Adam(model.classifier.parameters(), lr=self.args.lr, weight_decay=self.args.wd) 
            ...
            for epoch in range(self.args.ep):
                ...
                loss = model(self.features, self.features2, self.labels, self.idx_train, epoch, self.idx_test)
...
class modeler(nn.Module):
    ...
    def forward(self, features, features2, labels, idx_train, epoch, idx_test, finetune=False):
        ...
        embed2 = self.encoder(features2)
        embed2 = F.normalize(embed2, dim=1)
        loss_contra = compute_contra_loss(embed, embed2)
        loss_nodeclassification = loss_nodeclassification + 0.3 * loss_contra

        return loss_nodeclassification

def compute_contra_loss(embed, embed2):
    f = lambda x: torch.exp(x / 1.0) 
    refl_sim = f(get_sim(embed, embed))
    between_sim = f(get_sim(embed, embed2))
